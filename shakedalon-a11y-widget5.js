(function() {
  ...
  function loadState() {
    try {
      ...
      if (stored) {
        ...
        // If saved state is from an older version, discard it
        if (parsed._version !== WIDGET_VERSION) {
          localStorage.removeItem('sa11y-state');
          return;
        }
        state = Object.assign(state, parsed);
        state.pausedVideos = [];
        // Validate numeric values - ensure they are within valid ranges
        if (typeof state.fontSize !== 'number' || state.fontSize < 80 || state.fontSize > 200) state.fontSize = 100;
        if (typeof state.lineHeight !== 'number' || state.lineHeight < 100 || state.lineHeight > 300) state.lineHeight = 100;
        if (typeof state.letterSpacing !== 'number' || state.letterSpacing < 0 || state.letterSpacing > 10) state.letterSpacing = 0;
        if (typeof state.wordSpacing !== 'number' || state.wordSpacing < 0 || state.wordSpacing > 10) state.wordSpacing = 0;
      }
    } catch (e) {
      console.error('Failed to load accessibility state:', e);
      // If localStorage is corrupted, reset it
      try { localStorage.removeItem('sa11y-state'); } catch(err) {}
    }
  }

  // Save state to localStorage
  var WIDGET_VERSION = '3.1';
  function saveState() {
    try {
      const stateToSave = Object.assign({}, state);
      delete stateToSave.pausedVideos;
      stateToSave._version = WIDGET_VERSION;
  ...
  // Reading guide - interactive, follows mouse AND touch
  function toggleReadingGuide(enabled) {
    if (enabled) {
      ...
      document.addEventListener('touchmove', moveReadingGuideTouch, { passive: true });
    } else {
      var guide = document.getElementById('sa11y-reading-guide');
      if (guide) guide.style.display = 'none';
      document.removeEventListener('mousemove', moveReadingGuide);
      document.removeEventListener('touchmove', moveReadingGuideTouch);
    }
  }
  ...
  function moveReadingGuideTouch(e) {
    if (e.touches && e.touches.length > 0) {
      var guide = document.getElementById('sa11y-reading-guide');
      if (guide) guide.style.top = (e.touches[0].clientY - 4) + 'px';
    }
  }
  ...
  function resetAll() {
    ...
    // Clean up reading guide (mouse + touch)
    toggleReadingGuide(false);
    var guide = document.getElementById('sa11y-reading-guide');
  ...
  function showAccessibilityModal() {
    ...
    content.style.cssText = 'background:white;border-radius:8px;max-width:600px;width:calc(100% - 32px);max-height:80vh;overflow-y:auto;padding:30px;box-shadow:0 20px 60px rgba(0,0,0,0.3);direction:rtl;';
